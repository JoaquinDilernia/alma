...
## Imágenes (no usan Firebase Storage)

Las fotos de producto, guarniciones, hero y categorías se sirven como **estáticos desde `public/images/`** (los sube Hostinger junto con el resto del sitio), **no** desde Firebase Storage. Se hizo así porque servirlas desde Storage facturaba ~2,4 TB/mes de egress (~USD 233 en agosto 2026).

- El panel **no** tiene subida de imágenes.
- La asociación registro→foto vive en código: `lib/imagenesProductos.js` (por id de Firestore) y `lib/imagenesGuarniciones.js`.
- **Para agregar la foto de un producto nuevo:** guardá la imagen optimizada (WebP, máx ~1000px, <200 KB) en `public/images/productos/<archivo>.webp` y agregá la línea correspondiente en `lib/imagenesProductos.js`. Después `npm run build` y subir `out/`.
- `scripts/migrar-imagenes.mjs` fue la migración one-off que bajó las fotos originales de Storage, las recomprimió y generó los manifiestos. Necesita `ffmpeg`. Se puede volver a correr si hace falta regenerar todo.
