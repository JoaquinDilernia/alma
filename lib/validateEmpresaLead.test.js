...
describe("validateEmpresaLead", () => {
  ...
  it("rejects an unknown modalidad", () => {
    const result = validateEmpresaLead({ ...validData, modalidad: "otra-cosa" });
    expect(result.valid).toBe(false);
    expect(result.errors.modalidad).toBeDefined();
  });

  it("rejects a missing or zero cantidadPersonas", () => {
    expect(validateEmpresaLead({ ...validData, cantidadPersonas: "" }).errors.cantidadPersonas).toBeDefined();
    expect(validateEmpresaLead({ ...validData, cantidadPersonas: "0" }).errors.cantidadPersonas).toBeDefined();
  });

  it("rejects an unknown frecuencia", () => {
    const result = validateEmpresaLead({ ...validData, frecuencia: "cuando pinte" });
    expect(result.valid).toBe(false);
    expect(result.errors.frecuencia).toBeDefined();
  });

  it("does not require a mensaje", () => {
    const { mensaje, ...rest } = validData;
    expect(validateEmpresaLead(rest).valid).toBe(true);
  });
});
